...
class TextFieldFloatingActionButton extends StatefulWidget {
  ...
  TextFieldFloatingActionButton(this.label, this.icon,
      {this.onChange, this.onSubmit, this.onClear, this.backgroundColor, this.iconColor});

  @override
  _TextFieldFloatingActionButtonState createState() =>
      _TextFieldFloatingActionButtonState(label, icon, onChange: onChange, onSubmit: onSubmit, onClear: onClear);
}

class _TextFieldFloatingActionButtonState extends State<TextFieldFloatingActionButton> {
  final String label;
  final IconData icon;
  final Color backgroundColor;
  final Color iconColor;
  ...
  _TextFieldFloatingActionButtonState(this.label, this.icon,
      {this.onChange, this.onSubmit, this.onClear, this.backgroundColor = Colors.white, this.iconColor = Colors.black});
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
        ...
        decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(32), color: backgroundColor, boxShadow: kElevationToShadow[6]),
        child: Row(
          children: [
            Expanded(
                child: Container(
                    ...
                    child: !folded
                        ? TextField(
                            decoration: InputDecoration(
                                hintText: label, hintStyle: TextStyle(color: iconColor), border: InputBorder.none),
